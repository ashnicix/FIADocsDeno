...
const run = async (): Promise<embed[]> => {
  ...
  documents.toArray().forEach((document) => {
    ...
    if ("attribs" in document && "href" in document.attribs) {
      ...
      elem.date =
        $(`a[href="${document.attribs.href}"] .published .date-display-single`)
          .text().trim() + " CET";
...
const sliceEmbedArr = (arr: embed[]): embed[] => {
  return arr.sort((a, b) => {
    return moment(a.footer.text, "DD.MM.YY hh:mm").unix() -
...
const newEmbed = ({ title, date, event, url }: element): embed => {
  return {
    ...
    url: url,
    thumbnail: {
      url: Config.get().thumbnail ??
        "https://static.ort.dev/fiadontsueme/fia_logo.png",
    },
    description: "",
    footer: {
